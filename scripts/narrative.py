...
def _gemini(prompt):
    """無料Geminiで生成。429は待って数回リトライ（無料枠の分間制限対策）。"""
        ...
        for attempt in range(3):
            try:
                ...
                if e.code == 429 and attempt < 2:
                    time.sleep(20 * (attempt + 1))  # 分間制限が空くのを待つ
                    continue
                print(f"[warn] gemini {model} (HTTP {e.code})")
                break
    ...
    if sources:
        ...
        if out and out.get("background"):
            return {"background": out["background"].strip(),
                    ...
                    "sources": src_meta, "method": "gemini"}

    # フォールバック（出典なし or LLM不可・429）。method=mechanical＝次回再生成対象
    return {"background": _mechanical_background(card),
            "outlook": "開示・報道の反映待ち。次に業種内の比較と通期見通しを確認。",
            "sources": src_meta, "method": "mechanical"}
